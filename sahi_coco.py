...
def export_coco_as_yolov5(
    ...
    numpy_seed=0,
    mod_train_dir: str = None,
    mod_val_dir: str = None
):
    """
    Exports current COCO dataset in ultralytics/yolov5 format.
    Creates train val folders with image symlinks and txt files and a data yaml file.
    ...
    # create yolov5 data yaml
    if mod_train_dir is not None and mod_val_dir is not None:
        data = {
            "train": str(Path(os.path.abspath(mod_train_dir)) / "train/"),
            "val": str(Path(os.path.abspath(mod_val_dir)) / "val/"),
